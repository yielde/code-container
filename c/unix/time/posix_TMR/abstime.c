...
int main(int argc, char *argv[]) {
  ...
  if ((argc < 2) || (argc > 5) || (data.type < 0)) {
    fprintf(stderr, "Usage: %s -r | -a | -p [inctime [numtimes [spintime]]]\n",
            argv[0]);
    return 1;
  }

  if (argc > 2) data.inctime = atof(argv[2]);
  if (argc > 3) data.numtimes = atoi(argv[3]);
  if (argc > 4) data.spintime = atof(argv[4]);
  ...
  if ((sigemptyset(&act.sa_mask) == -1) ||
    ...
    perror("Failed to set handler for SIGALARM");
    return 1;
  }
  ...
  if (clock_getres(CLOCK_REALTIME, &clockres) == -1)
    ...
    fprintf(stderr, "Clock resolution is %0.3f ms or %0.9f s\n",
