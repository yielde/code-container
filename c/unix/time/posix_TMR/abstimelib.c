...
static int spinit(double stime) {
  struct timespec tcurrent;
  double tend, tnow;
  if (stime == 0.0) return 0;
  if (clock_gettime(CLOCK_REALTIME, &tcurrent) == -1) return -1;
...
void timehandler(int signo, siginfo_t *info, void *centext) {
  timer_data *datap;
  ...
  if (datap->type == TYPE_PERIODIC) return;
