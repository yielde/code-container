...
void del_resource() {
  cout << "clean resource sigpipe fd" << endl;
  close(sig_pipefd[0]);
  close(sig_pipefd[1]);
  cout << "clean resource listenfd" << endl;
  close(listenfd);
  cout << "clean resource epollfd" << endl;
  close(epollfd);
  cout << "clean resource sharememory" << endl;
  shm_unlink(shm_name);
  delete[] users;
  delete[] sub_process;
}
            ...
            switch (signals[i]) {
              case SIGCHLD: {
                ...
                while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
                  cout << "wait " << pid << " exit" << endl;
                  int del_user = sub_process[pid];
                  cout << "del user" << del_user << endl;
                  sub_process[pid] = -1;
                  cout << "sub process pid" << sub_process[pid] << endl;
